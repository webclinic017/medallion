...
def backtestRunner(Strategy, symbols, provider, candle_size, start_date, end_date, init_balance, stratOpts):
  ...
  #print('start_date:', start_date, 'current_datetime:', current_datetime)
  progress_date = current_datetime.strftime('%Y-%m-%d')
  sys.stdout.write('\r'+progress_date)
  while current_datetime < end_date:
    if current_datetime.strftime('%Y-%m-%d') != progress_date:
      progress_date = current_datetime.strftime('%Y-%m-%d')
      sys.stdout.flush()
      sys.stdout.write('\r'+progress_date+' Balance: '+str(int(trader.balance)))
      if current_datetime.strftime('%Y-%m-%d') == end_date.strftime('%Y-%m-%d'):
        print('')
    for symbol in symbols:
